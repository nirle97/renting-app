...
const setLikeStatus = async (req: Decoded, res: Response): Promise<void> => {
    ...
    try {
        await AptModel.updateLikeStatus(req.params.aptId, req.decoded.id, req.query.status.toString())
        await UserModel.findOneAndUpdate({ _id: req.decoded.id }, {$push: {"likedApts": req.params.aptId}}, {new: true})
...
const aptController = { addNewApt, setLikeStatus, getAptsByFilters, getAptsByOwner }
